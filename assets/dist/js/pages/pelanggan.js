$(function () {
	...
	var myTable = $("#datatables").DataTable({
		...
		ajax: {
			url: `${base_url}data_pelanggan/getList`,
			type: "POST",
			data: function (d) {
...
function addNew() {
	$("#form").trigger("reset");
	$(".modalHeaderText").html("Add New Pelanggan");
	showModal();
...
function view(e) {
	...
	jQuery.ajax({
		...
		url: `${base_url}data_pelanggan/getOne`,
		dataType: "json",
		success: function (e) {
			$(".modalHeaderText").html(`Detail Pelanggan ${e[0].nama}`);
			var d = new Date();
			var t = d.getTime();
			$("#body-view").html(
				`<div class="row">
...
function edit(e) {
	$(".modalHeaderText").html("Edit Pelanggan");
	showModal();
	jQuery.ajax({
		type: "post",
		data: {
		...
		dataType: "json",
		success: function (e) {
			jQuery("#id_pelanggan").val(e[0].id_pelanggan);
...
function deleteData(uid) {
	Swal.fire({
		...
		if (result.value) {
			var myForm = $("#form")[0];
			$.ajax({
				url: `${base_url}data_pelanggan/delete`,
				type: "POST",
				data: {
					uid: uid,
				},
